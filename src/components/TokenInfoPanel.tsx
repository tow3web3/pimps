...
function SocialLink({ href, label }: { href: string; label: string }) {
  return (
    <a
      ...
      className="chip hover:!text-[var(--cyan)] hover:!border-[rgba(34,211,238,0.4)] transition-colors"
    >
      {label} ↗
    </a>
  );
  ...
  return (
    <div className="glass flex flex-col min-h-0 overflow-hidden">
      <div className="flex items-center gap-2 px-3 py-2.5 border-b border-[var(--border)]">
        ...
        {info?.graduated && (
          <span className="chip !text-[var(--up)] !border-[rgba(52,211,153,0.35)]">
            ✓ graduated
          </span>
        )}
      </div>
        ...
        {loading && (
          <div className="p-4 space-y-2">
            {Array.from({ length: 4 }).map((_, i) => (
              <div key={i} className="h-6 rounded bg-[rgba(140,160,255,0.05)] animate-pulse" />
            ))}
          </div>
        )}
          ...
          <div className="p-3 space-y-3">
            ...
            <div className="flex flex-wrap gap-1.5">
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                className="chip hover:!text-[var(--up)] hover:!border-[rgba(52,211,153,0.4)] transition-colors"
              >
                pump.fun ↗
              </a>
              {info.pairAddress && (
