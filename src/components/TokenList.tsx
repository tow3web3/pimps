...
function TokenIcon({ url, symbol, size = 26 }: { url?: string; symbol: string; size?: number }) {
  ...
  return (
    <div
      className="rounded-full border border-[var(--border)] bg-[rgba(242,239,230,0.08)] flex items-center justify-center shrink-0 mono text-[10px] text-[var(--ink-2)]"
      style={{ width: size, height: size }}
    >
        ...
        {!universeLoaded && (
          <div className="p-4 space-y-3">
            {Array.from({ length: 8 }).map((_, i) => (
              <div key={i} className="h-9 rounded-lg bg-[rgba(242,239,230,0.05)] animate-pulse" />
            ))}
          </div>
        )}
